...
class Sitemap {
	...
	private function getFilenamebydir($directory,$i) {
		$files = glob($directory.'*.xml');
		return  basename($files[$i], ".xml");
	}/**
...
/**
	 ...
	 */
	public function createSitemapIndex($loc,$directory, $lastmod = 'Today') {
		//$this->endSitemap();
		...
		for ($index = 0; $index < $this->filecount($directory); $index++) {
			$indexwriter->startElement('sitemap');
			$indexwriter->writeElement('loc', $loc . $this->getFilenamebydir($directory, $index). self::EXT);
