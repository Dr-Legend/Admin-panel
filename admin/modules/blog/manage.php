        ...
        <table width="100%" class="table table-bordered table-striped manage-table"  id="sort" cellpadding="0" cellspacing="0">
          <thead>
            <tr class="tbl-head">
              ...
              <th width="3%">#</th>
              <th width="32%" class="noPad2"> <a href="<?=$ADMIN->iurl($comp)?>&fld=name<?=(($otype=='asc')?"&otype=desc":'&otype=asc')?>" <?=(($fld=='name')?'class="selectedTab"':'')?>><span <?=(($otype=='asc')?'class="des"':'class="asc"')?>>Name</span></a> </th>
                <th width="20%" class="noPad2"> <a href="<?=$ADMIN->iurl($comp)?>&fld=blogDate<?=(($otype=='asc')?"&otype=desc":'&otype=asc')?>" <?=(($fld=='blogDate')?'class="selectedTab"':'')?>><span <?=(($otype=='asc')?'class="des"':'class="asc"')?>>Publish Date</span></a> </th>
