<?php 
 ...
 $RW->redir($ADMIN->iurl('audio_album'), true);
  }
  $PAGS = new Pages();
  if($action)
  {
 ...
 <?=$subpage_id?'Track':$ADMIN->breadcrumb($comp)?></a></span>   
        ...
        <table width="100%" class="table table-bordered table-striped manage-table"  id="sort" cellpadding="0" cellspacing="0">
          <thead>
            <tr class="tbl-head">
              ...
              <th width="3%">#</th>
              <th width="37%" class="noPad2"> <a href="<?=$ADMIN->iurl($comp)?>&fld=name<?=(($otype=='asc')?"&otype=desc":'&otype=asc')?>" <?=(($fld=='name')?'class="selectedTab"':'')?>><span <?=(($otype=='asc')?'class="des"':'class="asc"')?>>Track Title</span></a> </th>
            <th width="37%" class="noPad2"> <a href="<?=$ADMIN->iurl($comp)?>&fld=name<?=(($otype=='asc')?"&otype=desc":'&otype=asc')?>" <?=(($fld=='name')?'class="selectedTab"':'')?>><span <?=(($otype=='asc')?'class="des"':'class="asc"')?>>Track</span></a> </th>
  ...
  ?>
            <tr class="<?=$css?>" id="recordsArray_<?=$pid?>">
             ...
             <td><audio controls="" class="play_pause">
