        ...
        <table width="100%" class="table table-bordered table-striped manage-table"  id="sort" cellpadding="0" cellspacing="0">
          <thead>
            <tr class="tbl-head">
              ...
              <th width="3%">#</th>
              <th width="32%" class="noPad2"> <a href="<?=$ADMIN->iurl($comp)?>&fld=name<?=(($otype=='asc')?"&otype=desc":'&otype=asc')?>" <?=(($fld=='name')?'class="selectedTab"':'')?>><span <?=(($otype=='asc')?'class="des"':'class="asc"')?>>Mission Title</span></a> </th>
                <th width="20%" class="noPad2"> <a href="<?=$ADMIN->iurl($comp)?>&fld=missionDate<?=(($otype=='asc')?"&otype=desc":'&otype=asc')?>" <?=(($fld=='missionDate')?'class="selectedTab"':'')?>><span <?=(($otype=='asc')?'class="des"':'class="asc"')?>>Mission Date</span></a> </th>
             
              <th width="20%" class="noPad2"> <a href="<?=$ADMIN->iurl($comp)?>&fld=missionTime<?=(($otype=='asc')?"&otype=desc":'&otype=asc')?>" <?=(($fld=='missionTime')?'class="selectedTab"':'')?>><span <?=(($otype=='asc')?'class="des"':'class="asc"')?>>Mission Time</span></a> </th>
