...
<div class="card mb-3">
  ...
  <div class="card-body">
    <div class="form-group row border-bottom pb-3">
      <label class="col-12  col-md-3">Category Name<span class="text-danger">*</span></label>
      <div class="col-12 col-md-9">
    <select name="category_id"  class="form-control" data-parsley-trigger="change" data-parsley-required-message="category is required!" required>
  
  <option  value="">-------Select Category------</option>
  <?php $bry=$PDO->db_query("select * from #_category where status=1");
  while($brs=$PDO->db_fetch_array($bry)){
   ?>
  <option value="<?=$brs['pid']?>" <?=($category_id==$brs['pid'])?'selected="selected"':''?>  ><?=$brs['name']?></option>
  <?php }?>
  </select>
      </div>
    </div>
    <div class="form-group row border-bottom pb-3">
      <label class="col-12 col-md-3" for="heading">Video Title<span class="text-danger">*</span></label>
      <div class="col-12 col-md-9">
      <input type="text" name="name" data-parsley-trigger="change" value="<?=$name?>" required class="form-control" data-parsley-required-message="Please insert Video Title">
      </div>
    </div>
    <div class="form-group row border-bottom pb-3">
      <label class="col-12 col-md-3">Youtube Video Id</label>
      <div class="col-12 col-md-9">
      <input type="text" name="youtubevideoid" data-parsley-trigger="change" value="<?=$youtubevideoid?>"  required class="form-control" data-parsley-required-message="Please insert Youtube Video Id">
      </div>
    </div>
 <div class="form-group row border-bottom pb-3">
      <label class="col-12 col-md-3">Date</label>
      <div class="col-12 col-md-9">
      <input type="text" name="videoDate" data-parsley-trigger="change" value="<?=$videoDate?>" required  class="form-control datepicker" data-parsley-required-message="Please insert Date">
      </div>
    </div>
   <div class="form-group row border-bottom pb-3">
      <label class="col-12 col-md-3">Thumbnail</label>
      <div class="col-12 col-md-9">
