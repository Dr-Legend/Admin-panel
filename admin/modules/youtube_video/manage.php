...
<div class="card mb-3">
 ...
 <?=$ADMIN->alert()?> 
 <span class="pull-right"><a href="<?=$ADMIN->iurl($comp,'add'.(($catid)?'&catid='.$catid:'').(($bible_id)?'&bible_id='.$bible_id:''))?>" class="btn btn-primary btn-sm"><i class="fa fa-plus" aria-hidden="true"></i> Add new <?=$ADMIN->breadcrumb($comp)?></a></span>   
        ...
        <table width="100%" class="table table-bordered table-striped manage-table"  id="sort" cellpadding="0" cellspacing="0">
          <thead>
            <tr class="tbl-head">
              ...
              <th width="3%">#</th>
              <th width="22%" class="noPad2"> <a href="<?=$ADMIN->iurl($comp)?>&fld=category_id<?=(($otype=='asc')?"&otype=desc":'&otype=asc')?>" <?=(($fld=='category_id')?'class="selectedTab"':'')?>><span <?=(($otype=='asc')?'class="des"':'class="asc"')?>> Category Name</span></a> </th> 
              <th width="32%" class="noPad2"> <a href="<?=$ADMIN->iurl($comp)?>&fld=name<?=(($otype=='asc')?"&otype=desc":'&otype=asc')?>" <?=(($fld=='name')?'class="selectedTab"':'')?>><span <?=(($otype=='asc')?'class="des"':'class="asc"')?>> Video Title</span></a> </th>
                <th width="10%" class="noPad2"> <a href="<?=$ADMIN->iurl($comp)?>&fld=youtubevideoid<?=(($otype=='asc')?"&otype=desc":'&otype=asc')?>" <?=(($fld=='youtubevideoid')?'class="selectedTab"':'')?>><span <?=(($otype=='asc')?'class="des"':'class="asc"')?>> Youtube Video Id</span></a> </th>
             <th width="10%" class="noPad2"> <a href="<?=$ADMIN->iurl($comp)?>&fld=videoDate<?=(($otype=='asc')?"&otype=desc":'&otype=asc')?>" <?=(($fld=='videoDate')?'class="selectedTab"':'')?>><span <?=(($otype=='asc')?'class="des"':'class="asc"')?>> Date</span></a> </th>  
  ...
  ?>
            <tr class="<?=$css?>" id="recordsArray_<?=$pid?>">
              ...
              <td class="index"><?=$nums?></td>
               <td><?=$PDO->getSingleresult("select name from #_category where pid='".$category_id."'")?></td>
              <td><?=$name?></td>
              <td><?=$youtubevideoid?></td>
              <td><?=$videoDate?></td>
              <td><?=$ADMIN->displaystatus($status)?></td>
              <td class="action"><?=$ADMIN->action($comp.(($catid)?'&catid='.$catid:'').(($bible_id)?'&bible_id='.$bible_id:''), $pid)?></td>
            </tr>
            <?php $nums++; } ?>
          </tbody>
