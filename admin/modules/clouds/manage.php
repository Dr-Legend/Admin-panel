 ...
 <?=$ADMIN->alert()?> 
        ...
        <table width="100%" class="table table-bordered table-striped manage-table"  id="sort" cellpadding="0" cellspacing="0">
          <thead>
            <tr class="tbl-head">
              ...
              <th width="3%">#</th>
              <th width="12%" class="noPad2"> <a href="<?=$ADMIN->iurl($comp)?>&fld=name<?=(($otype=='asc')?"&otype=desc":'&otype=asc')?>" <?=(($fld=='name')?'class="selectedTab"':'')?>><span <?=(($otype=='asc')?'class="des"':'class="asc"')?>>Title</span></a> </th>
              <th width="75%" class="noPad2">Clouds Description</th>
  ...
  ?>
            <tr class="<?=$css?>" id="recordsArray_<?=$pid?>">
              ...
              <td class="action"><?=$ADMIN->actionEX($comp.(($catid)?'&catid='.$catid:''), $pid)?></td>
            </tr>
            <?php $nums++; } ?>
          </tbody>
