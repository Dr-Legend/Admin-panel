	...
	# Developer by Ajay Kumar Maurya
	*/
	...
	{
	@extract($data);
	$query=parent::db_query("select * from ".tb_Prefix."admin_users where email ='".$email."' "); 
	if($query->rowCount()==0)
	{
	$data['password']=$this->password($data['password']);
	$data['created_on']=@date('Y-m-d H:i:s');
	$data['create_by']=$_SESSION["AMD"][0];
	$data['shortorder']=parent::getSingleresult("select max(shortorder) as shortorder from ".tb_Prefix."admin_users where 1=1 ")+1;
	...
	{
	@extract($data);
	$query=parent::db_query("select * from ".tb_Prefix."admin_users where email ='".$email."' and user_id!='".$updateid."' "); 
	if($query->rowCount()==0)
	{
	//$data['password']=$this->password($data['password']);
	// parent::sqlquery("rs",'admin_users',$data);
	$data['modified_on']=@date('Y-m-d H:i:s');
	parent::sqlquery("rs",'admin_users',$data,'user_id',$updateid);
	...
	parent::db_query("delete from ".tb_Prefix."admin_users where user_id in ($updateid)");
	}
	...
	parent::db_query("update  ".tb_Prefix."admin_users set status='".$status."' where user_id in ($updateid)");
	}
	...
	$sql = " from ".tb_Prefix."admin_users where 1 ".$zone.$mtype.$extra.$extra1.$extra2.$wh;
