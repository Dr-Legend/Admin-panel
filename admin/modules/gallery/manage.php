...
<div class="card mb-3">
 ...
 <?=$subpage_id?'Image':$ADMIN->breadcrumb($comp)?></a></span>   
    <h3><i class="fa fa-table"></i>   <?=$PDO->getSingleresult("select name from #_gallery where pid='".$subpage_id."'")?></h3>
   </div>
        ...
        <table width="100%" class="table table-bordered table-striped manage-table"  id="sort" cellpadding="0" cellspacing="0">
          <thead>
            <tr class="tbl-head">
              ...
              <th width="3%">#</th>
              <th width="40%" class="noPad2"> <a href="<?=$ADMIN->iurl($comp)?>&fld=name<?=(($otype=='asc')?"&otype=desc":'&otype=asc')?>" <?=(($fld=='name')?'class="selectedTab"':'')?>><span <?=(($otype=='asc')?'class="des"':'class="asc"')?>> Title</span></a> </th>
             <?php if($subpage_id==0){?> 
  ...
  ?>
            <tr class="<?=$css?>" id="recordsArray_<?=$pid?>">
             ...
             <?php if($subpage_id==0){?>
                <td> <a href="index.php?comp=gallery&subpage_id=<?=$pid?>">Images (
                <?=$PDO->getSingleresult("select count(*) from #_gallery where subpage_id='".$pid."'")?>
                ) </a></td>
                <?php }?>
