        ...
        <table width="100%" class="table table-bordered table-striped manage-table"  id="sort" cellpadding="0" cellspacing="0">
          <thead>
            <tr class="tbl-head">
              ...
              <th width="3%">#</th>
              <th width="32%" class="noPad2"> <a href="<?=$ADMIN->iurl($comp)?>&fld=name<?=(($otype=='asc')?"&otype=desc":'&otype=asc')?>" <?=(($fld=='name')?'class="selectedTab"':'')?>><span <?=(($otype=='asc')?'class="des"':'class="asc"')?>>Title</span></a> </th>
                <th width="20%" class="noPad2"> <a href="<?=$ADMIN->iurl($comp)?>&fld=messageDate<?=(($otype=='asc')?"&otype=desc":'&otype=asc')?>" <?=(($fld=='messageDate')?'class="selectedTab"':'')?>><span <?=(($otype=='asc')?'class="des"':'class="asc"')?>>Message Date</span></a> </th>
             
              <th width="20%" class="noPad2"> <a href="<?=$ADMIN->iurl($comp)?>&fld=messageTime<?=(($otype=='asc')?"&otype=desc":'&otype=asc')?>" <?=(($fld=='messageTime')?'class="selectedTab"':'')?>><span <?=(($otype=='asc')?'class="des"':'class="asc"')?>>Message Time</span></a> </th>
