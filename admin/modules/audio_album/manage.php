<?php 
  ...
  if($subpage_id)
  {
  $extra = " and subpage_id='".$subpage_id."'   ";   	
  }else{
  $extra = " and subpage_id='0'";
  }
  $PAGS = new Pages();
  if($action)
  {
...
<div class="card mb-3">
 ...
 <?=$ADMIN->alert()?> 
 <span class="pull-right"><a href="<?=$ADMIN->iurl($comp,'add'.(($catid)?'&catid='.$catid:'').(($subpage_id)?'&subpage_id='.$subpage_id:''))?>" class="btn btn-primary btn-sm"><i class="fa fa-plus" aria-hidden="true"></i> Add new 
 
 <?=$subpage_id?'Track':$ADMIN->breadcrumb($comp)?></a></span>   
    <h3><i class="fa fa-table"></i>   <?=$PDO->getSingleresult("select name from #_audio_album where pid='".$subpage_id."'")?></h3>
   </div>
        ...
        <table width="100%" class="table table-bordered table-striped manage-table"  id="sort" cellpadding="0" cellspacing="0">
          <thead>
            <tr class="tbl-head">
              ...
              <th width="3%">#</th>
              <th width="40%" class="noPad2"> <a href="<?=$ADMIN->iurl($comp)?>&fld=name<?=(($otype=='asc')?"&otype=desc":'&otype=asc')?>" <?=(($fld=='name')?'class="selectedTab"':'')?>><span <?=(($otype=='asc')?'class="des"':'class="asc"')?>>Album Title</span></a> </th>
                <th width="10%" class="noPad2"> <a href="<?=$ADMIN->iurl($comp)?>&fld=albumDate<?=(($otype=='asc')?"&otype=desc":'&otype=asc')?>" <?=(($fld=='albumDate')?'class="selectedTab"':'')?>><span <?=(($otype=='asc')?'class="des"':'class="asc"')?>> Date</span></a> </th>  
             <?php if($subpage_id==0){?> 
              <th width="12%">Track List</th>
              <?php } ?>
  ...
  ?>
            <tr class="<?=$css?>" id="recordsArray_<?=$pid?>">
             ...
             <?php if($subpage_id==0){?>
                <td> <a href="index.php?comp=track_list&subpage_id=<?=$pid?>">Track List (
                <?=$PDO->getSingleresult("select count(*) from #_track_list where subpage_id='".$pid."'")?>
                ) </a></td>
                <?php }?>
