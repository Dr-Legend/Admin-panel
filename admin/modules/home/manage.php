 ...
 <span class="pull-right"><a href="<?=$ADMIN->iurl($comp,'add'.(($catid)?'&catid='.$catid:'').(($subpage_id)?'&subpage_id='.$subpage_id:''))?>" class="btn btn-primary btn-sm"><i class="fa fa-plus" aria-hidden="true"></i> Add new page</a></span>   
        ...
        <table width="100%" class="table table-bordered table-striped manage-table"  id="sort" cellpadding="0" cellspacing="0">
          <thead>
            <tr class="tbl-head">
              ...
              <th width="3%">#</th>
              <th width="20%" class="noPad2"> <a href="<?=$ADMIN->iurl($comp)?>&fld=pagename<?=(($otype=='asc')?"&otype=desc":'&otype=asc')?>" <?=(($fld=='pagename')?'class="selectedTab"':'')?>><span <?=(($otype=='asc')?'class="des"':'class="asc"')?>> Menu Name</span></a> </th>
              <th width="20%" class="noPad2"> <a href="<?=$ADMIN->iurl($comp)?>&fld=heading<?=(($otype=='asc')?"&otype=desc":'&otype=asc')?>" <?=(($fld=='heading')?'class="selectedTab"':'')?>><span <?=(($otype=='asc')?'class="des"':'class="asc"')?>> Heading</span></a> </th>
  ...
  ?>
            <tr class="<?=$css?>" id="recordsArray_<?=$pid?>">
              ...
              <td><?=$pagename?></td>
              <td><?=$heading?></td>
              <td><?php if($subpage_id==0){?>
                <a href="index.php?comp=home&subpage_id=<?=$pid?>">Sections (
                <?=$PDO->getSingleresult("select count(*) from #_home where subpage_id='".$pid."'")?>
                ) </a>
                <?php }?>
                <?php if($section==3){?>
                <a href="index.php?comp=home&subpage_id=<?=$pid?>">Tabs (
                <?=$PDO->getSingleresult("select count(*) from #_home where subpage_id='".$pid."'")?>
                ) </a>
                <?php }?></td>
              ...
              <td class="action">
              <?php if($subpage_id==0){?>
			  <?=$ADMIN->actionEX($comp.(($catid)?'&catid='.$catid:'').(($subpage_id)?'&subpage_id='.$subpage_id:''), $pid)?>
              <?php }else{?>
              <?=$ADMIN->action($comp.(($catid)?'&catid='.$catid:'').(($subpage_id)?'&subpage_id='.$subpage_id:''), $pid)?>
              <?php }?>
              </td>
            </tr>
            <?php $nums++; } ?>
