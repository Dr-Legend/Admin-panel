        ...
        <table width="100%" class="table table-bordered table-striped manage-table"  id="sort" cellpadding="0" cellspacing="0">
          <thead>
            <tr class="tbl-head">
              ...
              <th width="3%">#</th>
              <th width="20%" class="noPad2"> <a href="<?=$ADMIN->iurl($comp)?>&fld=bible_id<?=(($otype=='asc')?"&otype=desc":'&otype=asc')?>" <?=(($fld=='bible_id')?'class="selectedTab"':'')?>><span <?=(($otype=='asc')?'class="des"':'class="asc"')?>> Bible Name</span></a> </th>
              <th width="16%" class="noPad2"> <a href="<?=$ADMIN->iurl($comp)?>&fld=name<?=(($otype=='asc')?"&otype=desc":'&otype=asc')?>" <?=(($fld=='name')?'class="selectedTab"':'')?>><span <?=(($otype=='asc')?'class="des"':'class="asc"')?>> Chapter No</span></a> </th>
                <th width="16%" class="noPad2"> <a href="<?=$ADMIN->iurl($comp)?>&fld=paragraph<?=(($otype=='asc')?"&otype=desc":'&otype=asc')?>" <?=(($fld=='paragraph')?'class="selectedTab"':'')?>><span <?=(($otype=='asc')?'class="des"':'class="asc"')?>> Paragraph No</span></a> </th>
  ...
  ?>
            <tr class="<?=$css?>" id="recordsArray_<?=$pid?>">
              ...
              <td class="index"><?=$nums?></td>
               <td><?=$PDO->getSingleresult("select name from #_bible where pid='".$bible_id."'")?></td>
              <td><?=$name?></td>
              <td><?=$paragraph?></td>
